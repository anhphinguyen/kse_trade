...
if ($nums > 0) {
    ...
    $sql = "SELECT id,exchange_open,exchange_close,exchange_period,exchange_idle,exchange_updated_by FROM tbl_exchange_temporary WHERE id_exchange = '$id_exchange'";
    $result = db_qr($sql);
    $nums = db_nums($result);
    if ($nums > 0) {
        while ($row = db_assoc($result)) {
