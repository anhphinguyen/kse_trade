...
$sql = "SELECT 
                    ...
                    tbl_exchange_exchange.exchange_percent as exchange_percent
...
if ($nums > 0) {
    while ($row = db_assoc($result)) {
        $result_item = array(
            ...
            'period_close_int' => $row['period_close'],

            'period_now' => (isset($row['period_now']) && !empty($row['period_now'])) ? $row['period_now'] : "",

            'exchange_percent' => (isset($row['exchange_percent']) && !empty($row['exchange_percent'])) ? $row['exchange_percent'] : "",
