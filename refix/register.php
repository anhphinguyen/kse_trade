...
if (isset($_REQUEST['customer_authend']) && !(empty($_REQUEST['customer_authend']))) {
    if(is_username($_REQUEST['customer_authend'])){
        $customer_authend = $_REQUEST['customer_authend'];
    }else{
        returnError("Vui lòng chọn loại đăng kí");
    }
}
...
if (isset($_REQUEST['customer_cert_no']) && !(empty($_REQUEST['customer_cert_no']))) {
    if (is_cert($_REQUEST['customer_cert_no'])) {
        ...
        returnError("Vui lòng nhập số CMND");
    }
} else {
    returnError("Vui lòng nhập số CMND");
}
...
$sql = "INSERT INTO tbl_customer_customer SET 
        ...
        customer_authend = '$customer_authend', 
...
if (db_qr($sql)) {
    ...
    if ($nums > 0) {
        while ($row = db_assoc($result)) {
            ...
            if($row['customer_authend'] == 'N'){
                $title = "Thông báo có khách hàng đăng kí tài khoản";
                $bodyMessage = "Có khách hàng vừa đăng kí, đang chờ xác thực tài khoản.";
                $action = "customer_hasbeen_register";
                $type_send = 'topic';
                $to = 'KSE_customer_hasbeen_register';
