...
if (isset($_REQUEST['customer_authend'])) {
    if ($_REQUEST['customer_authend'] == '') {
        unset($_REQUEST['customer_authend']);
    } else {
        $customer_authend = htmlspecialchars($_REQUEST['customer_authend']);
        $sql .= " AND `customer_authend` = '$customer_authend'";
    }
}
$customer_disable = 'N';
...
if ($nums > 0) {
    while ($row = db_assoc($result)) {
        $customer_item = array(
            ...
            'customer_authend' => htmlspecialchars_decode($row['customer_authend']),
        );
