...
switch ($typeManager) {
    ...
    case 'cancel_request_payment': {
            ...
            $sql = "UPDATE tbl_request_payment SET
                    ...
                    ";
            if (db_qr($sql)) {
                $sql = "SELECT id_customer, request_value,request_type FROM tbl_request_payment WHERE id = '$id_request'";
                $result = db_qr($sql);
                $nums = db_nums($result);
                if ($nums > 0) {
                    while ($row = db_assoc($result)) {
                        ...
                        if ($nums > 0) {
                            while ($row = db_assoc($result)) {
                                if($request_type == '1'){
                                    ...
                                    $money_bet_update = $row['customer_wallet_rewards'] + $request_value;
                                    $money_payment_update = $row['customer_wallet_payment'] - $request_value;
                                    $sql_update = "UPDATE tbl_customer_customer SET 
                                                    customer_wallet_rewards = '$money_bet_update',
                                                    customer_wallet_payment = '$money_payment_update'
                                                    WHERE id = '$id_customer'
                                                    ";
                                }
